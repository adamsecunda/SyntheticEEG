...
class Generator(nn.Module):
    ...
    Takes a noise vector and class label and produces a synthetic 22x1001
    EEG epoch. The noise and class embedding are projected to an initial
    feature map of 256x7 and progressively upsampled through four transposed
    convolutional blocks to 22x1001. Dropout is applied after each block
    during training, and Gaussian noise is injected into the class embedding
    to prevent the generator producing trivially class-separable outputs.
    ...
    def __init__(self, noise_dim=NOISE_DIM, embed_dim=EMBED_DIM, n_classes=N_CLASSES):
        ...
        self.conv4 = nn.ConvTranspose1d(
            32, N_CHANNELS, kernel_size=14, stride=3, padding=0
        )

    def forward(self, z, labels):
        e = self.embed(labels)

        # Inject noise into class embedding during training to prevent
        # the generator producing trivially class-separable outputs
        if self.training:
...
class Critic(nn.Module):
    ...
    generator learning to fool a classifier-like discriminator. No batch
    normalisation is used as it interferes with the gradient penalty.
    ...
    def __init__(self, embed_dim=EMBED_DIM, n_classes=N_CLASSES, n_channels=N_CHANNELS):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(
                n_channels + 1, 32, kernel_size=3, stride=1, dilation=1, padding=1
            ),
            nn.LeakyReLU(0.2),
...
def _psd_loss(fake_x, real_x):
    ...
    density of the real data. Encourages the generator to produce signals
    with realistic frequency content in the mu and beta bands.
...
def train_gan(X, y, n_epochs=200, lr_g=0.00001, lr_d=0.0001, verbose=True):
    ...
    with realistic statistical and frequency properties. Instance noise
    is added to critic inputs and decayed over training to prevent the
    critic from becoming overconfident early in training.
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        noise_scale = 0.05 * (1 - epoch / n_epochs)
        ...
        for real_x, real_y in loader:
            ...
            for _ in range(N_CRITIC):
                ...
                # Instance noise added to critic inputs, decaying over training
                real_noisy = real_x + torch.randn_like(real_x) * noise_scale
                fake_noisy = fake_x + torch.randn_like(fake_x) * noise_scale

                gp = _gradient_penalty(critic, real_noisy, fake_noisy, real_y)
                c_loss = (
                    critic(fake_noisy, real_y).mean()
                    - critic(real_noisy, real_y).mean()
                    + LAMBDA_GP * gp
                )
